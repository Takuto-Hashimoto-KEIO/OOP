...
settings = ParameterSettings('takuto', 'B1', 1); % ()内は被験者番号、block番号、開始時の速度レベル(interval_index)
...
speed_change = SpeedChangeToReady(speed_changer, current_trial);
%%% speed_changeをどこに保存する？(objectがたくさんになり、内容が重複して管理しきれない)
speed_change = speed_change.trialStartToTask(); %%% なぜ()内にspeed_changerは不要なのか？
