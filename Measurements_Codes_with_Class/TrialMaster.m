classdef TrialMaster
    ...
    methods (Access = private)
        % block_type別の分岐操作、次trialの打鍵速度変更などを行う
        function  [obj, next_interval_index] = branching_by_block_type(obj, task_ev, cfg)

            switch cfg.block_type
                case 'S1' % Speed adjustment blockのみで2trialごとに、blockの終了判定と速度増加を実施（Screeening1のみ）
                    if mod(obj.current_trial, 2) == 0  % 偶数trialの終了時
                        S1_task_judger = TaskJudgerPer2Trials(obj.current_trial, obj.interval_index, task_ev.Results.success_duration, cfg.TrialTaskTime);
                ...
                case 'S2' % Speed adjustment blockのみで3trialごとに、速度増加を実施 & 9trial後に終了操作（Screeening2のみ）
                    ...
                    elseif mod(obj.current_trial, 3) == 0  % 3の倍数のtrialの終了時
                        next_interval_index = obj.interval_index + 1; % 次のtrialの打鍵速度の番号を設定
