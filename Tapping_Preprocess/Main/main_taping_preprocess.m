...
%% 読み込むフォルダパスを指定
folder_path = "C:\Users\takut\OneDrive - keio.jp\牛場研 M1~\修論研究\toolbox\Hashimoto Resarch\Progress 2\Results\20250114~ 予備実験4\20250306_Y421\tapping_data";
...
for fileIdx = 1:length(file_list)
    ...
    %% データの読みだし→処理
    block_data = TapProcessor();
    block_data = block_data.processing(cfg);

    %% trialの除去必要？→一旦無し

    %% 結果の保存
    saver = DataSaver(); % 保存処理のインスタンスを作成
    saved_path = saver.save_data(block_data, save_path); % (保存したいデータ, 保存先のパス)
end
...
%% データの保存
saved_path = saver.save_data(all_blocks_data, save_path); % (保存したいデータ, 保存先のパス) % 全データを保存
saved_path = saver.save_data(target_data, save_path); % (保存したいデータ, 保存先のパス) % 関心trialのみのデータを保存
